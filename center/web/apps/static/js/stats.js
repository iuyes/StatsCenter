    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (i = 0; i < data.stats.length; i++) {
                ...
                if (stats[interface_id]['max_time'] == undefined)
                {
                    stats[interface_id]['max_time'] = data.stats[i]['max_time'];
                }
                else
                {
                    if (data.stats[i]['max_time'] > stats[interface_id]['max_time'])
                    {
                        stats[interface_id]['max_time'] = data.stats[i]['max_time'];
                    }
                }

                if (stats[interface_id]['min_time'] == undefined)
                {
                    stats[interface_id]['min_time'] = data.stats[i]['min_time'];
                }
                else
                {
                    if (data.stats[i]['min_time'] < stats[interface_id]['min_time'])
                    {
                        stats[interface_id]['min_time'] = data.stats[i]['min_time'];
                    }
                }
                ...
                data.stats[i]['succ_count'] = data.stats[i]['total_count'] - data.stats[i]['fail_count'];
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                succ_count +=  parseInt(data[i]['total_count']-data[i]['fail_count'], 10);
            ...
            for (i = 0; i < data.length; i++) {
                ...
                stats[ip]['succ_count'] += parseInt(data[i]['total_count']-data[i]['fail_count'], 10);
